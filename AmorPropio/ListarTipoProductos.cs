...
{
    public partial class ListarTipoProductos : Form
    {
        int valorcelda = -1;
        public int Usuario { get; set; }
        public int Id_Rol { get; set; }
        public ListarTipoProductos()
        {
            InitializeComponent();
        }

        private void ListarTipoProductos_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void dat_principal_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    ListarTipoProductos_Load(null, null);
                }
                else
                {
        ...
        private void Renderizar(string Accion)
        {
            try
            {
                ...
                if (Accion != "A" && Accion == "M" && valorcelda != -1 || Accion != "A" && Accion == "E" && valorcelda != -1 || Accion != "A" && Accion == "C" && valorcelda != -1)
                {
                    ...
                    Mantenimiento_Tipos_Producto frm = new Mantenimiento_Tipos_Producto();
                    frm.Accion = Accion;
                    frm.Id = valorcelda;
                    frm.Usuario = Usuario;
                    frm.ShowDialog();
